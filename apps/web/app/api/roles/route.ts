...
export async function POST(req: Request) {
  ...
  // if client-provided id is a valid UUID, use it; otherwise generate a UUID on server
  let roleId: string;
  if (id && isValidUuid(id)) {
    ...
    // fallback (shouldn't happen in modern Node) — still return error instead of non-UUID
    return NextResponse.json({ error: "Server cannot generate UUID" }, { status: 500 });
  }
